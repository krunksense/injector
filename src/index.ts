...
const CHEAT_URL = debug ? 'http://localhost:2314' : 'https://krunk.cc/cheat';
...
app.whenReady().then(async () => {
    ...
    ipcMain.on('launch', (event, info) => {
        ...
        let launchWinPos = [mainWinPos[0] + mainWinSize[0] + 10, mainWinPos[1]];
...
function doLaunch(info: any, launchWindow: BrowserWindow) {
    ...
    launchWindow.webContents.once('did-finish-load', async () => {
        ...
        if (statSync(info.path).isFile() && extname(info.path) == '.AppImage') {
            ...
            if (!existsSync(join(copyTo, 'squashfs-root')))
                return (copyFiles = 'error');
        ...
        try {
            ...
            writeFileSync(
                join(appRoot, dirname, 'package.json'),
                oldPackageJSON
            );
            copyDir(join(__dirname, '../inject'), join(appRoot, dirname), {
                recursive: true,
            });
            inject = 'done';
            launchWindow.webContents.send('update-progress', 'inject', inject);
